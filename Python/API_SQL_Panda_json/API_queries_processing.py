""" Problem set 2 wrangling data
    Define a set of functions to implement SQL queries on data from CSV and 
    APIs
"""
import pandas
import pandasql
...
def num_rainy_days(filename):
    ...
    q = """SELECT count(*) FROM weather_data
           WHERE rain=1
        """
...
def max_temp_aggregate_by_fog(filename):
    ...
    q = """SELECT fog, cast(maxtempi as integer) FROM weather_data
           GROUP BY fog
        """
...
def fix_turnstile_data(filenames):
    ...
    for name in filenames:
        f_in == open(name,'r') # input file
        f_out = open('updated_' + name,'w') # output file
    
        reader_in = csv.reader(f_in, delimiter=',') # read in csv
    
        for line in reader_in:
            line = [i.replace(' ', "") for i in line] 
            header=line[0:3]
            for i in numpy.array(line[3:]).reshape(-1,5):
                row=header + list(i)
...
def filter_by_regular(filename):
    ...
    turnstile_data = pandas.DataFrame(pandas.read_csv(filename))
...
def reformat_subway_dates(date):
    ...
    date_formatted = date_1.strftime("%Y-%m-%d")
