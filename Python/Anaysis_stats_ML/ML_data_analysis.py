...
def entries_histogram(turnstile_weather):
    ...
    norain['ENTRIESn_hourly'].hist(alpha=0.75)
    rain['ENTRIESn_hourly'].hist(alpha=0.75,color='g')
    plt.xlabel("ENTRIESn_hourly",fontsize=15)
    plt.ylabel("Frequency",fontsize=15)
    plt.title("Histogram of ENTRIESn_hourly",fontsize=15)
...
def gradient_descent(features, values, theta, alpha, num_iterations):
    ...
    for i in range(num_iterations):
        Yi = np.dot(features,theta) # predicted values
    	# update theta
     	theta = theta - (alpha/m) * (np.dot((Yi - values),features)) 
...
def predictions(dataframe):
    ...
    alpha = 0.1 # lower values gave worse R2
    num_iterations = 75 # makes lttle diffence in this case
